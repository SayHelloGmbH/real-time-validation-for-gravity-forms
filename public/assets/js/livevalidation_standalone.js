...
LiveValidation.prototype = {
    ...
    initialize: function (element, optionsObj) {
        ...
        this.insertAfterWhatNode = node.nodeType
            ? node
        ...
        if (this.fieldType == "checkbox") {
            ...
            for (var i = 0; i < radioUL.childNodes.length; i++) {
                this.oldOnClick_check[
                    radioUL.childNodes[i].querySelector("input").id
                ] =
                    radioUL.childNodes[i].querySelector("input").onclick ||
                    function () {};
            }
        } else {
            this.oldOnClick_check = [];
                    ...
                    for (var i = 0; i < radioUL.childNodes.length; i++) {
                        radioUL.childNodes[i].querySelector("input").onclick =
                            function (e, i) {
                                ...
                                return self.oldOnClick_check[this.id].call(
                                    this,
                                    e
                                );
                            };
                        ...
                        radioUL.childNodes[i].querySelector("input").onfocus =
                            function (e) {
                                self.doOnFocus(e);
                                jqobjPrime(document).trigger(
                                    "lv_field_on_focus",
                                    e
                                );
                                return self.oldOnFocus.call(this, e);
                ...
                case LiveValidation.SELECT:
                    this.element.onblur = function (e) {
                        if (hasSubFields.indexOf(self.fieldType) == -1) {
                            ...
                            var elemLI = jqobjPrime(self.element).parents(
                                "li.gfield"
                            );
                            ...
                            elemLI.find("input , select").each(function (k, v) {
                                ...
                                if (
                                    typeof all_validations[getConfig[1]][
                                        getConfig[2] + "_" + getConfig[3]
                                    ] !== "undefined"
                                ) {
                                    all_validations[getConfig[1]][
                                        getConfig[2] + "_" + getConfig[3]
                                    ].validate();
                                }
                            });
    ...
    doValidations: function () {
        ...
        for (var i = 0, len = this.validations.length; i < len; ++i) {
            this.validationFailed = !this.validateElement(
                this.validations[i].type,
                this.validations[i].params
            );
    ...
    validateElement: function (validationFunction, validationParamsObj) {
        ...
        if (this.elementType == LiveValidation.RADIO) {
            ...
            if (
                validationFunction == Validate.Acceptance ||
                validationFunction == Validate.AcceptanceCheckbox
            ) {
    ...
    insertMessage: function (elementToInsert) {
        ...
        if (
            ...
            var className = this.validationFailed
                ? this.invalidClass
                : this.validClass;
            elementToInsert.className +=
...
LiveValidationForm.getInstance = function (element) {
    if (!element)
        throw new Error(
            "LiveValidationForm::getInstance - No element reference or element id has been provided!"
        );
            ...
            if (show_error_combined == true) {
                ...
                if (d.length > 0) {
                    ...
                    for (var i = 0; i < d.length; i++) {
                        ...
                        var label_elem = document.querySelector(
                            ".gfield_label[for='" + ID + "']"
                        );
                        if (!label_elem) {
                        ...
                        if (!label_elem) {
                            label_elem = document.querySelector(
                                "label[for='" + ID + "']"
                            );
                        }
                        ...
                        if (label_elem && error_message_elem) {
                            var error_message_text =
                                error_message_elem.textContent ||
                                error_message_elem.innerText;
                            var label_text1 =
                                label_elem.textContent || label_elem.innerText;
                            if (label_text1 == "" || label_text1 == "*") {
                                label_elem = document.querySelector(
                                    "#" +
                                        ID +
                                        "_container label[for='" +
                                        ID +
                                        "']"
                                );
                                var label_text1 =
                                    label_elem.textContent ||
                                    label_elem.innerText;
                            }

                            li.innerHTML =
                                "<span><a href='#' onclick='window.scrollTo(0, " +
                                jqobjPrime(li_elem).offset().top +
                                "); return false;'>" +
                                label_text1 +
                                "</a></span> " +
                                error_message_text +
...
var Validate = {
        ...
        if (typeof paramsObj.mask !== "undefined") {
            if (paramsObj.mask == value) {
                ...
                setTimeout(function () {
                    if (
                        !hasClass(
                            all_validations[paramsObj.form_id][
                                paramsObj.livevalidkey
                            ].element,
                            "LV_invalid_field"
                        )
                    ) {
    ...
    Numericality: function (value, paramsObj) {
        ...
        var paramsObj = paramsObj || {};
        var minimum =
            paramsObj.minimum || paramsObj.minimum == 0
                ? paramsObj.minimum
                : null;
        var maximum =
            paramsObj.maximum || paramsObj.maximum == 0
                ? paramsObj.maximum
                : null;
        var is = paramsObj.is || paramsObj.is == 0 ? paramsObj.is : null;
        var notANumberMessage =
            paramsObj.notANumberMessage || "Must be a number!";
        var notAnIntegerMessage =
            paramsObj.notAnIntegerMessage || "Must be an integer!";
        var wrongNumberMessage =
            paramsObj.wrongNumberMessage || "Must be " + is + "!";
        var tooLowMessage =
            paramsObj.tooLowMessage || "Must not be less than " + minimum + "!";
        var tooHighMessage =
    ...
    Format: function (value, paramsObj) {
        ...
        var paramsObj = paramsObj || {};
        var message =
            paramsObj.failureMessage || "Looks like an invalid pattern.";
    ...
    Email: function (value, paramsObj) {
        var paramsObj = paramsObj || {};
        var message =
            paramsObj.failureMessage || "Must be a valid email address!";
    ...
    Inclusion: function (value, paramsObj) {
        var paramsObj = paramsObj || {};
        var message =
            paramsObj.failureMessage || "Must be included in the list!";
    ...
    Exclusion: function (value, paramsObj) {
        var paramsObj = paramsObj || {};
        paramsObj.failureMessage =
            paramsObj.failureMessage || "Must not be included in the list!";
    ...
    AcceptanceCheckbox: function (value, paramsObj) {
        ...
        var get_mainUL =
            "input_" + paramsObj.form_id + "_" + paramsObj.field_id;
        ...
        for (var i = 0; i < radioUL.childNodes.length; i++) {
            if (radioUL.childNodes[i].querySelector("input").checked) {
                ...
                for (var j = 0; j < radioUL.childNodes.length; j++) {
                    removeClass(
                        radioUL.childNodes[j].querySelector("input"),
                        "LV_invalid_field"
                    );
                }
    ...
    AcceptanceRadio: function (value, paramsObj) {
        ...
        var get_mainUL =
            "input_" + paramsObj.form_id + "_" + paramsObj.name_field.substr(6);
        ...
        if (paramsObj.name_field) {
            ...
            for (var i = 0; i < inps.length; i++) {
                if (inps[i].checked) {
                    ...
                    if (typeof radioUL.classList == "undefined") {
                        removeClass(radioUL, "LV_invalid_field");
                        var inps = document.getElementsByName(
                            paramsObj.name_field
                        );
                        for (var i = 0; i < inps.length; i++) {
    ...
    if (newnumber.length == 10) {
        fld.val(
            "(" +
                ...
                "-" +
                newnumber.substring(6, newnumber.length)
        );
        alert(phonenumber);
        return true;
    } else if (newnumber.length == 0) {
