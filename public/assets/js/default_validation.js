    ...
    afterInvalid: function (elem) {
        ...
        if (elemLi.length > 0 && !jQuery(elemLi[0]).hasClass("gfield_error")) {
            ...
            if ($thiss.text().trim().length > 0) {
                ...
                htML +=
                    '<div class="gfield_description validation_message message_id_' +
                    jQuery(elem).attr("id") +
                    '">' +
                    get_error_label($thiss, elem) +
                    "" +
                    $thiss.text() +
                    "</div>";
            }
            ...
            if (
                typeof all_validations[getConfig[1]][get_match] !== "undefined"
            ) {
                if (
                    hasSubFields.indexOf(
                        all_validations[getConfig[1]][get_match].fieldType
                    ) > -1 &&
                    jQuery(elemLi[0]).find(
                        ".message_id_" + jQuery(elem).attr("id")
                    ).length == 0
                ) {
                    var htML = "";
