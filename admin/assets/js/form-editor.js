...
var supported_fields_pattern = new Array(
    "text",
    "textarea",
    "number",
    "name",
...
var supported_fields = new Array(
    "text",
    "textarea",
    "number",
    "checkbox",
...
for (key in supported_fields_pattern) {
    if (is_RealTimeValidOn) {
        fieldSettings[supported_fields_pattern[key]] +=
            ", .gf_live_validation_type_settings_level_1";
    }
}
...
function createbaseHash(elem) {
    try {
        ...
        document.getElementById("gf_live_validation_format_based").value = btoa(
            elem.value
        );
        SetFieldProperty(
            "gf_live_validation_format_based",
            document.getElementById("gf_live_validation_format_based").value
        );

        var clone = field["lv_validation"] || new Array();
        var All_fields = jQuery(
            ".gf_live_validation_type_settings_is_" +
                getDefaultInputField(field) +
                " .gf_lv_is_pattern_field"
        );
        var extra_hidden_data = jQuery(
            "#gf_live_validation_type_settings_is_" +
                getDefaultInputField(field) +
                "_hidden_data"
        );
        ...
        for (
            var incr = incr_init;
            incr < All_fields.length + incr_init;
            incr++
        ) {
            if (jQuery(All_fields[count_int]).attr("id") == elem.id) {
                ...
                elem_config.push({
                    ...
                    error: document.getElementById(
                        "gf_live_validation_hidden_error"
                    ).value,
                    ftype: jQuery(All_fields[count_int]).attr("data-ftype"),
                });
        ...
        jQuery("body").trigger(
            "lv_admin_apply_validations",
            document.getElementById("gf_live_validation_value").checked
        );
    } catch (err) {
...
function moveErrorTohidden(elem) {
    document.getElementById("gf_live_validation_hidden_error").value =
        elem.value;
    SetFieldProperty(
        "gf_live_validation_hidden_error",
        document.getElementById("gf_live_validation_hidden_error").value
...
jQuery(".lv_open_support_box").on("click", function () {
    tb_show(
        ...
        "#TB_inline?height=700&amp;width=500&amp;inlineId=my-help-div-" +
            getDefaultInputField(field)
    );
});
...
jQuery("#field_required").on("change", function (event) {
    if (this.checked == true && jQuery("#field_error_message").val() == "") {
        jQuery("#field_error_message").val(
            lv_default_error_messages[field["type"]]
        );
    }
        ...
        switch (getDefaultInputField(field)) {
            case "date":
                if (field.dateType !== "datefield") {
                    if (this.checked == true) {
                        jQuery(
                            ".gf_live_validation_type_settings_valid"
                        ).slideDown();
                        jQuery(
                            ".gf_live_validation_type_settings_is_date"
                        ).slideUp();
                        jQuery(
                            ".gf_live_validation_type_settings_is_date_error"
                        ).slideUp();
                    } else {
                        jQuery(
                            ".gf_live_validation_type_settings_valid"
            ...
            case "name":
                if (this.checked == true) {
                    jQuery(".gf_live_validation_type_settings_valid").slideUp();
                    jQuery(
                        ".gf_live_validation_type_settings_is_name"
                    ).slideDown();
                    jQuery(
                        ".gf_live_validation_type_settings_is_name_error"
                    ).slideDown();
                } else {
                    jQuery(".gf_live_validation_type_settings_valid").slideUp();
                    jQuery(
            ...
            case "address":
                if (this.checked == true) {
                    jQuery(".gf_live_validation_type_settings_valid").slideUp();
                    jQuery(
                        ".gf_live_validation_type_settings_is_address"
                    ).slideDown();
                    jQuery(
                        ".gf_live_validation_type_settings_is_address_error"
                    ).slideDown();
                } else {
                    jQuery(".gf_live_validation_type_settings_valid").slideUp();
                    jQuery(
            ...
            case "time":
                if (this.checked == true) {
                    jQuery(".gf_live_validation_type_settings_valid").slideUp();
                    jQuery(
                        ".gf_live_validation_type_settings_is_time"
                    ).slideDown();
                    jQuery(
                        ".gf_live_validation_type_settings_is_time_error"
                    ).slideDown();
                } else {
                    jQuery(".gf_live_validation_type_settings_valid").slideUp();
                    jQuery(
    ...
    if (this.checked == true) {
        ...
        for (
            ...
            if (field["isRequired"] == true) {
                elem_config.push({
                    type: "presence",
                    ftype: jQuery(All_fields[count_opr]).attr("data-ftype"),
                });
            }
            elem_config.push({
                type: "pattern",
                format: btoa(jQuery(All_fields[count_opr]).val()),
                error: jQuery(All_fields_error[count_opr]).val(),
                ftype: jQuery(All_fields[count_opr]).attr("data-ftype"),
            });
            clone[incr] = elem_config;
            count_opr++;
        ...
        if (field["isRequired"] == true) {
            ...
            if (
                typeof field["lv_validation"] !== "undefined" &&
                field["lv_validation"].length > 0
            ) {
                ...
                for (
                    ...
                    clone_valid[incr] = [
                        {
                            type: "presence",
                            ftype: jQuery(All_fields[count_opr]).attr(
                                "data-ftype"
                            ),
                        },
                    ];
...
jQuery(document).bind(
    ...
    function (event, field, form) {
        jQuery("#gf_live_validation_format").attr(
            "data-ftype",
            getDefaultPattern(getDefaultInputField(field))
        );

        if (
            supported_fields_pattern.indexOf(getDefaultInputField(field)) ==
                -1 &&
            fields_inherit.indexOf(field.type) == -1
        ) {
            return true;
        }
        ...
        jQuery("#gf_live_validation_format").val(
            field["gf_live_validation_format"]
        );
        jQuery("#gf_live_validation_format").trigger("keyup");
        jQuery("#gf_live_validation_format_based").val(
            field["gf_live_validation_format_based"]
        );
        jQuery("#gf_live_validation_error_msg").val(
            field["gf_live_validation_error_msg"]
        );

        jQuery("#gf_live_validation_value").attr(
            "checked",
            field["gf_live_validation_settings"] == true
        );
...
jQuery("body").on("lv_admin_apply_validations", function (event, var1) {
    ...
    switch (getDefaultInputField(field)) {
            ...
            if (field.dateType == "datepicker") {
                ...
                if (var1 == true) {
                    field_Arr.push({
                        ...
                        error: document.getElementById(
                            "gf_live_validation_error_msg"
                        ).value,
                        ftype: jQuery("#gf_live_validation_format").attr(
                            "data-ftype"
                        ),
                    });
                }
        ...
        case "email":
            ...
            field["lv_validation"][0] = field_Arr;
            if (
                document.getElementById("gfield_email_confirm_enabled")
                    .checked == true
            ) {
        ...
        case "address":
            if (field.addressType !== "international") {
                if (field["isRequired"] == true) {
                    field["lv_validation"][3] = new Array({
                        type: "presence",
                        ftype: "select",
                    });
                ...
                if (field["isRequired"] == true) {
                    if (typeof field["lv_validation"][5] == "undefined") {
                        field["lv_validation"].push(
                            new Array({ type: "presence", ftype: "select" })
                        );
                    }
                }
        ...
        default:
            ...
            if (
                ...
                (field["inputMask"] == false ||
                    typeof field["inputMask"] == "undefined")
            ) {
                field_Arr.push({
                    type: "pattern",
                    format: document.getElementById(
...
function getDefaultInputField(field_custom) {
    if (
        typeof field_custom["inputType"] !== "undefined" &&
        field_custom["inputType"] !== ""
    ) {
